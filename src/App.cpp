...
public:
	...
	void initialise(const String& commandLine) override
	{
		...
		mainWindow = std::make_unique<MainHostWindow>();
	...
	void handleAsyncUpdate() override
	{
		...
		{
			fileToOpen =
			    File::getCurrentWorkingDirectory().getChildFile(getCommandLineParameterArray()[i]);
...
{
	return autoScaleOptionAvailable && (description.pluginFormatName.containsIgnoreCase("VST") ||
...
void addPluginAutoScaleOptionsSubMenu(const AudioPluginInstance* pluginInstance, PopupMenu& menu)
{
	...
	autoScaleMenu.addItem("Enabled", true,
	                      getAutoScaleValueForPlugin(identifier) == AutoScale::scaled, [identifier]
	                      { setAutoScaleValueForPlugin(identifier, AutoScale::scaled); });

	autoScaleMenu.addItem("Disabled", true,
	                      getAutoScaleValueForPlugin(identifier) == AutoScale::unscaled, [identifier]
