...
{
public:
	explicit InternalPlugin(std::unique_ptr<AudioProcessor> innerIn) : inner(std::move(innerIn))
	{
...
protected:
	...
	{
		return inner->setBusesLayout(layouts) && AudioPluginInstance::applyBusLayouts(layouts);
	}
...
private:
	AudioProcessorEditor* createEditor() override
	{
		...
		{
		public:
			explicit Decorator(InternalPlugin& proc) : AudioProcessorEditor(proc), self(proc)
			{
			...
			~Decorator() override { self.editorBeingDeleted(this); }

			void paint(Graphics&) override { }

			void resized() override { wrapped->setBounds(getLocalBounds()); }
			...
			bool wantsLayerBackedView() const override { return wrapped->wantsLayerBackedView(); }

			bool usesWindowsMultiTouch() const override { return wrapped->usesWindowsMultiTouch(); }
...
const std::vector<PluginDescription>& InternalPluginFormat::InternalPluginFactory::getDescriptions() const
{
	return descriptions;
}

std::unique_ptr<AudioPluginInstance>
InternalPluginFormat::InternalPluginFactory::createInstance(const String& name) const
{
	const auto begin = descriptions.begin();
	const auto it = std::find_if(begin, descriptions.end(), [&](const PluginDescription& desc)
	                             { return name.equalsIgnoreCase(desc.name); });

	if (it == descriptions.end())
		return nullptr;

	const auto index = static_cast<size_t>(std::distance(begin, it));
...
String InternalPluginFormat::getIdentifier()
{
	return "Internal";
}
...
bool InternalPluginFormat::fileMightContainThisPluginType(const String&)
{
	return true;
}

FileSearchPath InternalPluginFormat::getDefaultLocationsToSearch()
{
	return {};
}
...
void InternalPluginFormat::findAllTypesForFile(OwnedArray<PluginDescription>&, const String&)
{
}

bool InternalPluginFormat::doesPluginStillExist(const PluginDescription&)
{
	return true;
}

String InternalPluginFormat::getNameOfPluginFromIdentifier(const String& fileOrIdentifier)
{
	return fileOrIdentifier;
}

bool InternalPluginFormat::pluginNeedsRescanning(const PluginDescription&)
{
	return false;
}

StringArray InternalPluginFormat::searchPathsForPlugins(const FileSearchPath&, bool, bool)
{
	return {};
}
