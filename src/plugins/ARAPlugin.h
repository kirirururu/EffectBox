...
public:
	...
	bool readAudioSamples(float* const* buffers, int64 startSample, int64 numSamples)
	{
		...
		return formatReader->read(buffers, (int)formatReader->numChannels, startSample,
		                          (int)(numSamples));
	}
...
private:
	using Converter =
	    ARAHostModel::ConversionFunctions<MusicalContext*, ARA::ARAMusicalContextHostRef>;
...
private:
	using Converter =
	    ARAHostModel::ConversionFunctions<RegionSequence*, ARA::ARARegionSequenceHostRef>;
...
private:
	using Converter =
	    ARAHostModel::ConversionFunctions<AudioModification*, ARA::ARAAudioModificationHostRef>;
...
class PlaybackRegion
{
	auto getPlaybackRegionProperties() const
	{
		...
		properties.durationInModificationTime =
		    (double)formatReader.lengthInSamples / formatReader.sampleRate;
...
private:
	using Converter =
	    ARAHostModel::ConversionFunctions<PlaybackRegion*, ARA::ARAPlaybackRegionHostRef>;
...
public:
	                      ...
	                      void* const* buffers) noexcept override
	{
		...
		auto* audioSource =
		    FileAudioSource::Converter::fromHostRef(Converter::fromHostRef(readerRef)->sourceHostRef);

		if (use64BitSamples)
			return audioSource->readAudioSamples(reinterpret_cast<double* const*>(buffers),
			                                     samplePosition, samplesPerChannel);

		return audioSource->readAudioSamples(reinterpret_cast<float* const*>(buffers),
		                                     samplePosition, samplesPerChannel);
	}
...
public:
	using ReaderConverter =
	    ARAHostModel::ConversionFunctions<MemoryBlock*, ARA::ARAArchiveReaderHostRef>;
	using WriterConverter =
	    ARAHostModel::ConversionFunctions<MemoryOutputStream*, ARA::ARAArchiveWriterHostRef>;
	...
	ARA::ARAPersistentID
	getDocumentArchiveID(ARA::ARAArchiveReaderHostRef archiveReaderHostRef) noexcept override
	{
...
public:
	using Converter =
	    ARAHostModel::ConversionFunctions<ARA::ARAContentType, ARA::ARAContentReaderHostRef>;
	...
	ARA::ARAContentGrade
	getMusicalContextContentGrade(ARA::ARAMusicalContextHostRef musicalContextHostRef,
	                              ARA::ARAContentType type) noexcept override
	{
	...
	ARA::ARAInt32
	getContentReaderEventCount(ARA::ARAContentReaderHostRef contentReaderHostRef) noexcept override
	{
		const auto contentType = Converter::fromHostRef(contentReaderHostRef);

		if (contentType == ARA::kARAContentTypeTempoEntries ||
...
public:
	...
	void
	notifyAudioModificationContentChanged(ARA::ARAAudioModificationHostRef audioModificationHostRef,
	                                      const ARA::ARAContentTimeRange* range,
	                                      ARA::ContentUpdateScopes scopeFlags) noexcept override
	{
...
public:
	...
	void requestSetCycleRange(ARA::ARATimePosition startTime,
...
class AudioSourceComponent final : public Component, public FileDragAndDropTarget, public ChangeListener
{
public:
	explicit AudioSourceComponent(HostPlaybackController& controller, juce::ChangeBroadcaster& bc)
	    : hostPlaybackController(controller), broadcaster(bc), waveformComponent(*this)
	{
		audioSourceLabel.setText("You can drag and drop .wav files here",
	...
	~AudioSourceComponent() override { broadcaster.removeChangeListener(this); }

	void changeListenerCallback(ChangeBroadcaster*) override { update(); }
	...
	void resized() override
	{
		...
		fb.items = {juce::FlexItem(goToStartButton)
		                .withMinWidth(100.0f)
		                .withMinHeight((float)buttonsArea.getHeight()),
		            juce::FlexItem(playButton)
		                .withMinWidth(100.0f)
		                .withMinHeight((float)buttonsArea.getHeight())};
...
private:
	class WaveformComponent final : public Component, public ChangeListener
	{
	public:
		WaveformComponent(AudioSourceComponent& p)
		    : parent(p), thumbCache(7), audioThumb(128, formatManager, thumbCache)
		{
		...
		~WaveformComponent() override { audioThumb.removeChangeListener(this); }
...
{
public:
	class ARATestHost final : public HostPlaybackController, public juce::ChangeBroadcaster
	{
	public:
		...
		explicit ARATestHost(ARAPluginInstanceWrapper& instanceIn) : instance(instanceIn)
		{
		...
		void init(ARAFactoryWrapper araFactory)
		{
			if (araFactory.get() != nullptr)
			{
				documentController = ARAHostDocumentController::create(
				    std::move(araFactory), "AudioPluginHostDocument",
				...
				if (documentController != nullptr)
				{
					...
					const auto plugInExtensionInstance =
					    documentController->bindDocumentToPluginInstance(*instance.inner, allRoles,
					                                                     allRoles);
		...
		void afterProcessBlock(int numSamples)
		{
			...
			if (isPlayingNow)
			{
				...
				// Rudimentary attempt to not seek beyond our sample data, assuming a fairly stable
				// numSamples value. We should gain control over calling the AudioProcessorGraph's
				// processBlock() calls so that we can do sample precise looping.
	...
	private:
		/**  Use this to put the plugin in an unprepared state for the duration of adding and
		   removing PlaybackRegions to and from Renderers.
		*/
		...
		public:
			...
			explicit ContextUpdateSource(const File& file)
			    : type(Type::audioSourceFile), audioSourceFile(file)
			{
			}

			ContextUpdateSource(const void* d, int s)
			    : type(Type::stateInformation), stateInformation(d, (size_t)s)
			{
			}

			ContextUpdateSource(Type t) : type(t) { jassert(t == Type::reset); }
		...
		void synchronizeStateWithDocumentController()
		{
			...
			auto newContext = [&]() -> std::unique_ptr<Context>
			{
				...
				case ContextUpdateSource::Type::stateInformation:
					jassert(contextUpdateSource.getStateInformation().getSize() <=
					        std::numeric_limits<int>::max());

					return Context::createFromStateInformation(
			...
			{
				{
					...
					audioSourceLength.store(
					    context->fileAudioSource.getFormatReader().lengthInSamples);
		...
		struct Context
		{
			...
			createFromStateInformation(ARA::Host::DocumentController& dc, const void* d, int s)
			{
