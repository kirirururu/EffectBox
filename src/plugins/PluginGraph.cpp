...
{
	return shouldAutoScalePlugin(desc) ? std::make_unique<ScopedDPIAwarenessDisabler>() : nullptr;
}
...
PluginWindow* PluginGraph::getOrCreateWindowFor(AudioProcessorGraph::Node* node,
...
void PluginGraph::setLastDocumentOpened(const File& file)
{
	...
	getAppProperties().getUserSettings()->setValue("recentFilterGraphFiles", recentFiles.toString());
}

//==============================================================================
static void readBusLayoutFromXml(AudioProcessor::BusesLayout& busesLayout,
                                 AudioProcessor& plugin,
                                 const XmlElement& xml,
                                 bool isInput)
{
	auto& targetBuses = (isInput ? busesLayout.inputBuses : busesLayout.outputBuses);
	int maxNumBuses = 0;
	...
	if (auto* buses = xml.getChildByName(isInput ? "INPUTS" : "OUTPUTS"))
	{
		for (auto* e : buses->getChildWithTagNameIterator("BUS"))
		{
			...
			// the number of buses on busesLayout may not be in sync with the plugin after adding
			// buses because adding an input bus could also add an output bus
...
static XmlElement* createBusLayoutXml(const AudioProcessor::BusesLayout& layout, const bool isInput)
{
	...
	for (int busIdx = 0; busIdx < buses.size(); ++busIdx)
	{
		...
		bus->setAttribute("layout",
		                  set.isDisabled() ? "disabled" : set.getSpeakerArrangementAsString());
	}
...
static XmlElement* createNodeXml(AudioProcessorGraph::Node* const node) noexcept
{
	if (auto* plugin = dynamic_cast<AudioPluginInstance*>(node->getProcessor()))
	{
		...
		for (int i = 0; i < (int)PluginWindow::Type::numTypes; ++i)
		{
			...
			{
				e->setAttribute(PluginWindow::getLastXProp(type),
				                node->properties[PluginWindow::getLastXProp(type)].toString());
				e->setAttribute(PluginWindow::getLastYProp(type),
				                node->properties[PluginWindow::getLastYProp(type)].toString());
				e->setAttribute(PluginWindow::getOpenProp(type),
				                node->properties[PluginWindow::getOpenProp(type)].toString());
			}
		}
...
void PluginGraph::createNodeFromXml(const XmlElement& xml)
{
	...
	if (auto instance = createInstanceWithFallback())
	{
		...
		if (auto node =
		        graph.addNode(std::move(instance), NodeID((uint32)xml.getIntAttribute("uid"))))
		{
			...
			for (int i = 0; i < (int)PluginWindow::Type::numTypes; ++i)
			{
				...
				{
					node->properties.set(PluginWindow::getLastXProp(type),
					                     xml.getIntAttribute(PluginWindow::getLastXProp(type)));
					node->properties.set(PluginWindow::getLastYProp(type),
					                     xml.getIntAttribute(PluginWindow::getLastYProp(type)));
					node->properties.set(PluginWindow::getOpenProp(type),
					                     xml.getIntAttribute(PluginWindow::getOpenProp(type)));
...
void PluginGraph::restoreFromXml(const XmlElement& xml)
{
	...
	{
		graph.addConnection(
		    {{NodeID((uint32)e->getIntAttribute("srcFilter")), e->getIntAttribute("srcChannel")},
