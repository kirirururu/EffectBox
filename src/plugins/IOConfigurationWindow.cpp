...
//==============================================================================
struct NumberedBoxes final : public TableListBox, private TableListBoxModel, private Button::Listener
{
	struct Listener
	{
		virtual ~Listener() { }
...
private:
	...
	Component*
	refreshComponentForCell(int, int columnId, bool, Component* existingComponentToUpdate) override
	{
		...
		const bool isPlusMinusButton =
		    (columnId == plusButtonColumnId || columnId == minusButtonColumnId);
		...
		else
		{
			...
			textButton->setColour(TextButton::buttonOnColourId,
			                      busColour.withMultipliedBrightness(2.0f));
		}
	...
	void updateBusLayout()
	{
		if (auto* plugin = owner.getAudioProcessor())
		{
			if (auto* bus = plugin->getBus(isInput, currentBus))
			{
				...
				enabledToggle.setToggleState(bus->isEnabled(),
		...
		if (btn == &enabledToggle && enabledToggle.isEnabled())
		{
			if (auto* p = owner.getAudioProcessor())
			{
				if (auto* bus = p->getBus(isInput, currentBus))
				{
					...
					{
						bool success = enabledToggle.getToggleState()
						                   ? bus->enable()
...
IOConfigurationWindow::IOConfigurationWindow(AudioProcessor& p)
    : AudioProcessorEditor(&p), title("title", p.getName())
{
	setOpaque(true);
